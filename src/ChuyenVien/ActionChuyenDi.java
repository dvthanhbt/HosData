...
public class ActionChuyenDi {
    public ResultSet getListChuyenDi(Date tuNgay, Date denNgay)throws Exception{
        String sql="CALL CHUYENVIEN.LISTCHUYENDI(?, ?)";
        PreparedStatement ps = DataUtil.getConnect().prepareStatement(sql);
        ps.setDate(1, (Date) tuNgay);
        ps.setDate(2, (Date) denNgay);
        ResultSet rs;
        rs = ps.executeQuery();
        return rs;
    }
    ...
    public String saveData(ChuyenDi cd)throws Exception{
        ...
        cs.setDate(2, (Date) cd.getNgayChuyen());
        ...
        cs.setDate(15, (Date) cd.getTuNgay());
        cs.setDate(16, (Date) cd.getDenNgay());
