...
public class ActionXaPhuong {
    ...
    public ResultSet getXaPhuong(String maQH){
        String sql = "SELECT * FROM HETHONG.XAPHUONG WHERE MAQH = '" + maQH + "' ORDER BY MAXP";
    ...
    public void saveXaPhuong(XaPhuong xaPhuong) throws SQLException{
        String sql = "CALL HETHONG.SAVEXAPHUONG (?, ?, ?, ?)";
        PreparedStatement ps = DataUtil.getConnect().prepareStatement(sql);
        ps.setString(1, xaPhuong.getMaXP());
        ps.setString(2, xaPhuong.getMaQH());
        ps.setString(3, xaPhuong.getTenXP());
        ps.setString(4, xaPhuong.getTenTat());
        ps.execute();
    }
}
